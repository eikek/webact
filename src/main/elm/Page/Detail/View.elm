...
import Data.Param exposing (Format(..), formatToString, stringToFormat)
...
view model =
    let
        ...
        out =
            Maybe.andThen .output model.info

        lastmod =
            Maybe.map Data.Script.lastMod script
    ...
    div []
        ...
        , Html.map DeleteConfirmMsg (Comp.YesNoDimmer.view model.deleteConfirm)
        , labels model
        , div []
...
renderTabs model =
    let
        tabs =
            List.map (tabContent model) [ Run, Content, Stdout, Stderr ]
    in
    div []
                    ...
                    div [ class "ui basic segment" ]
                        (fields
                            ++ [ div [ class "sixteen wide field" ]
                                    ...
                                    , input
                                        [ type_
                                            (if Data.Argument.isInput arg Line then
                                                "text"
...
categoryTags tags =
    List.map
        (\c ->
            div [ class "ui tag label", title "Category" ]
                [ text c ]
        )
        tags
