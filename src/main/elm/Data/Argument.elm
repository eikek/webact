...
makeEmptyArg name format index =
    case format of
        File -> Upl { name = name, content = [], index = index, input = File }
        Files -> Upl { name = name, content = [], index = index, input = Files }
        _ -> Str { name = name, content = "", index = index, input = format }
...
setIndex index arg =
    let
        set1 = (\a -> Str {a | index = index})
        set2 = (\a -> Upl {a | index = index})
    in
        fold set1 set2 arg
...
setName name arg =
    let
        set1 = (\a -> Str {a | name = name})
        set2 = (\a -> Upl {a | name = name})
    in
        fold set1 set2 arg
...
replaceArg arg list =
    ...
    in
    List.map
        (\e -> if (getIndex e) == index then arg else e)
        list
